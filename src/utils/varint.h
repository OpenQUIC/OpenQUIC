...
#define quic_varint_r2(payload)  \
    quic_varint_len(payload) == 2 ? (quic_bswap_16((*(uint16_t *) (payload)) & ~0xC0)) :

#define quic_varint_r4(payload)  \
    quic_varint_len(payload) == 4 ? (quic_bswap_32((*(uint32_t *) (payload)) & ~0xC0)) :

#define quic_varint_r8(payload)  \
    quic_varint_len(payload) == 8 ? (quic_bswap_64((*(uint64_t *) (payload)) & ~0xC0)) : 0
...
#define quic_varint_format_r(buf, payload) {                                   \
    ...
    if (len == 1) {                                                            \
        ...
        *(uint16_t *) (buf)->pos = quic_bswap_16((uint16_t) (payload)) | 0x40; \
    }                                                                          \
    else if (len == 4) {                                                       \
        *(uint32_t *) (buf)->pos = quic_bswap_32((uint32_t) (payload)) | 0x80; \
    }                                                                          \
    else if (len == 8) {                                                       \
        *(uint64_t *) (buf)->pos = quic_bswap_64((payload)) | 0xC0;            \
    }                                                                          \
    (buf)->pos += len;                                                         \
}
