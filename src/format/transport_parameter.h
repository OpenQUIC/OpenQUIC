...
__quic_header_inline size_t quic_transport_parameter_size(const quic_transport_parameter_t params) {
    return 2
...
__quic_header_inline bool quic_transport_parameter_varint_format(quic_buf_t *const buf, uint16_t id, uint64_t value) {
    uint16_t varint_len = quic_varint_format_len(value);
    if (buf->buf + buf->capa < buf->pos + 4 + varint_len) {
        return false;
    }

    *((uint16_t *) buf->pos) = quic_bswap_16(id);
    buf->pos += 2;
    *((uint16_t *) buf->pos) = quic_bswap_16(varint_len);
    buf->pos += 2;
...
__quic_header_inline bool quic_transport_parameter_string_format(quic_buf_t *const buf, uint16_t id, quic_buf_t value) {
    if (quic_buf_size(&value)) {
        ...
        *((uint16_t *) buf->pos) = quic_bswap_16(id);
        buf->pos += 2;
        *((uint16_t *) buf->pos) = quic_bswap_16(quic_buf_size(&value));
        buf->pos += 2;
...
__quic_header_inline bool quic_transport_parameter_bool_format(quic_buf_t *const buf, uint16_t id, bool value) {
    if (value) {
...
__quic_header_inline quic_err_t quic_transport_parameter_format(quic_buf_t *const buf, const quic_transport_parameter_t params) {
    ...
    *len = quic_bswap_16(buf->pos - buf->buf);

    return quic_err_success;
}

__quic_header_inline quic_buf_t quic_transport_parameter_string_parse(quic_buf_t *const buf) {
    quic_buf_t ret;
    ret.capa = quic_bswap_16(*(uint16_t *) buf->pos);
    buf->pos += 2;
    if (ret.capa > (size_t) quic_buf_size(buf)) {
        return ret;
    }
    ret.buf = quic_malloc(ret.capa);
    if (!ret.buf) {
        return ret;
    }
...
__quic_header_inline uint64_t quic_transport_parameter_varint_parse(quic_buf_t *const buf) {
    buf->pos += 2;
...
__quic_header_inline bool quic_transport_parameter_bool_parse(quic_buf_t *const buf) {
    buf->pos += 2;
    return true;
}

__quic_header_inline quic_transport_parameter_t quic_transport_parameter_parse(quic_buf_t *const buf) {
    quic_transport_parameter_t ret = { };
    quic_transport_parameter_init(&ret);

    uint16_t len = quic_bswap_16(*(uint16_t *) buf->pos);
    buf->pos += 2;
    len -= 2;
    ...
    while (quic_buf_size(buf) >= 4) {
        uint16_t id = quic_bswap_16(*(uint16_t *) buf->pos);
        buf->pos += 2;
